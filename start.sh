...
if [ "$#" -eq 0 ] || [ $1 = "-h" ] || [ $1 = "--help" ]; then
    echo "Usage: ./start [OPTIONS] COMMAND [arg...]"
    echo "       ./start [ -h | --help ]"
    echo ""
    echo "Options:"
...
stop_existing() {
  MYAPP="$(docker ps --all --quiet --filter=name=mock-pl)"
  REDIS="$(docker ps --all --quiet --filter=name=mock-pl-redis)"
  MONGO="$(docker ps --all --quiet --filter=name=mock-pl-mongo)"
